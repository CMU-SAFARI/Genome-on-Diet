...
static ko_longopt_t long_options[] = {{"bucket-bits", ko_required_argument, 300},
                                      ...
                                      {"vt_dis", ko_required_argument, 352},
                                      {"no-hash-name", ko_no_argument, 353},
                                      {"vt_nb_loc", ko_required_argument, 354},
                                      {"vt_cov", ko_required_argument, 355},
                                      {"max_max_gap", ko_required_argument, 356},
                                      {"max_min_gap", ko_required_argument, 357},
                                      {"vt_f", ko_required_argument, 358},
                                      {"vt_df1", ko_required_argument, 359},
                                      {"vt_df2", ko_required_argument, 360},
...
static inline int64_t mm_parse_num(const char *str) { return mm_parse_num2(str, 0); }
...
int main(int argc, char *argv[]) {
	const char *opt_str = "2aSDw:k:K:t:r:f:Vv:g:G:I:d:XT:s:x:Hcp:M:n:z:A:B:O:E:m:N:Qu:R:hF:LC:yYPo:e:U:Z:W:i:";
	...
	while ((c = ketopt(&o, argc, argv, 1, opt_str, long_options)) >=
	       0) { // test command line options and apply option -x/preset first
		if (c == 'x') {
	...
	opt.vt_df2      = 0.01;
	opt.vt_f        = 0.05;
	opt.max_max_gap = 50000;
	opt.min_dp_max = 40;
	opt.max_min_gap = 4000;
	ipt.pattern_len = 2;
	ipt.pattern     = "11";
	...
	while ((c = ketopt(&o, argc, argv, 1, opt_str, long_options)) >= 0) {
		if (c == 'w')
			...
			opt.pattern_len = atoi(o.arg);
			ipt.pattern_len = atoi(o.arg);
		} // give pattern length
		else if (c == 'i') {
			opt.max_seeds =
			    strtod(o.arg, NULL); // max number of seeds for aligning the pattern with the read
			if (opt.max_seeds < 0) {
				opt.max_seeds = 0.1;
			}
		} else if (c == 'H')
			...
			opt.max_frag_len = (int)mm_parse_num2(o.arg, &s);
			if (*s == ',') {
				opt.max_nb_rounds = strtol(s + 1, &s, 10);
				if (opt.max_nb_rounds < 1) {
					fprintf(stderr,
					        "[ERROR]: Max number of rounds should be greater than or equal to 1\n");
					exit(1);
				}
			}
		} else if (c == 'N')
			...
			opt.flag |= MM_F_ALL_CHAINS | MM_F_NO_DIAG | MM_F_NO_DUAL |
			            MM_F_NO_LJOIN; // -D -P --no-long-join --dual=no
		else if (c == 'a')
			...
			char *p;
			opt.min_cnt = strtod(o.arg, &p);
			if (*p == ',') {
				opt.rec_threshold_frac = strtod(p + 1, &p);
			}
		} else if (c == 'm')
			...
			if (strcmp(o.arg, "-") != 0) {
				if (freopen(o.arg, "wb", stdout) == NULL) {
					fprintf(
					    stderr,
					    "[ERROR]\033[1;31m failed to write the output to file '%s'\033[0m: %s\n",
			...
			opt.vt_dis = strtoul(o.arg, NULL, 10);
		} else if (c == 353)
			opt.flag |= MM_F_NO_HASH_NAME; // --no-hash-name
		else if (c == 354) {
			opt.vt_nb_loc = strtoul(o.arg, NULL, 10);
		} else if (c == 355) {
			opt.vt_cov = strtof(o.arg, NULL);
		} else if (c == 356) {
			opt.max_max_gap = strtoul(o.arg, NULL, 10);
		} else if (c == 357) {
			opt.max_min_gap = strtoul(o.arg, NULL, 10);
		} else if (c == 358) {
			opt.vt_f = strtof(o.arg, NULL);
		} else if (c == 359) {
			opt.vt_df1 = strtof(o.arg, NULL);
		} else if (c == 360) {
			opt.vt_df2 = strtof(o.arg, NULL);
		} else if (c == 330) {
			...
			if (o.arg == 0 || strcmp(o.arg, "short") == 0) {
				...
				fprintf(stderr, "[WARNING]\033[1;31m --cs only takes 'short' or 'long'. Invalid values "
				                "are assumed to be 'short'.\033[0m\n");
			}
		} else if ((opt.pattern_len < 2) || (ipt.pattern_len < 2)) {
			opt.pattern_len = 2;
			ipt.pattern_len = 2; // disable the pattern
			opt.pattern     = "11";
			ipt.pattern     = "11";
		} else if (c == 319) { // --splice-flank
			yes_or_no(&opt, MM_F_SPLICE_FLANK, o.longidx, o.arg, 1);
		} else if (c == 324) { // --heap-sort or radix-sort
			if (strcmp(o.arg, "radix") == 0) {
				opt.flag |= MM_F_RADIX_SORT;
				opt.flag &= ~MM_F_HEAP_SORT;
			} else if (strcmp(o.arg, "heap") == 0) {
				opt.flag |= MM_F_HEAP_SORT;
				opt.flag &= ~MM_F_RADIX_SORT;
			} else if (strcmp(o.arg, "merge") == 0) {
				opt.flag &= ~MM_F_HEAP_SORT;
				opt.flag &= ~MM_F_RADIX_SORT;
			} else {
				fprintf(stderr, "[ERROR]: Unknown sort algorithm (please use: merge, radix or heap)\n");
				exit(1);
			}
		} else if (c == 326) { // --dual
			...
			if (mm_verbose >= 2)
				fprintf(stderr, "[WARNING]\033[1;31m option -S is deprecated and may be removed in "
				                "future. Please use --cs=long instead.\033[0m\n");
		} else if (c == 'V') {
			puts(MM_VERSION);
			return 0;
		} else if (c == 'r') {
			opt.bw = strtoul(o.arg, NULL, 10);
		} else if (c == 'U') {
			...
			if (*o.arg == 'b')
				...
				opt.flag |= MM_F_SPLICE_REV,
				    opt.flag &= ~MM_F_SPLICE_FOR; // match CT-AC (reverse complement of GT-AG)
			else if (*o.arg == 'n')
	...
	if ((opt.flag & MM_F_SPLICE) && (opt.flag & MM_F_FRAG_MODE)) {
		fprintf(stderr,
		        "[ERROR]\033[1;31m --splice and --frag should not be specified at the same time.\033[0m\n");
		return 1;
	}
	if (!fnw && !(opt.flag & MM_F_CIGAR)) ipt.flag |= MM_I_NO_SEQ;
	if (mm_check_opt(&ipt, &opt) < 0) return 1;
	if (opt.best_n == 0) {
	...
	if (argc == o.ind || fp_help == stdout) {
		fprintf(fp_help, "Genome-on-Diet\n");
		fprintf(fp_help, "=======Usage=======\n");
		fprintf(fp_help, "Wihout AVX acceleraion:                    GDiet [options] <target.fa>|<target.idx> [query.fa] [...]\n");
		fprintf(fp_help, "With AVX acceleration:                     GDiet_avx [options] <target.fa>|<target.idx> [query.fa] [...]\n");
		fprintf(fp_help, "With AVX-accelerated Indexing/Sketching:   GDiet_sketch_avx [options] <target.fa>|<target.idx> [query.fa] [...]\n");
		fprintf(fp_help, "With AVX-accelerated Sequencing Alignment: GDiet_ksw2_avx [options] <target.fa>|<target.idx> [query.fa] [...]\n");
		fprintf(fp_help, "Options:\n");
		fprintf(fp_help, "=======Compressed Indexing=======\n");
		fprintf(fp_help, "    -H                 use homopolymer-compressed k-mer (preferrable for PacBio)\n");
		fprintf(fp_help, "    -k INT             k-mer size (no larger than 28) [%d]\n", ipt.k);
		fprintf(fp_help, "    -w INT             minimizer window size [%d]\n", ipt.w);
		fprintf(fp_help, "    -I NUM             split index for every ~NUM input bases [4G]\n");
		fprintf(fp_help, "    -d FILE            dump index to FILE []\n");
		fprintf(fp_help, "    -Z STR             diet pattern [%s]\n", ipt.pattern);
		fprintf(fp_help, "    -W INT             length of diet pattern [%d]\n", ipt.pattern_len);
		fprintf(fp_help, "=======Pattern Alignment=======\n");
		fprintf(fp_help, "    -i FLOAT           maximum number of minimizers (as absolute number when i>=1 or fraction \n"
		                 "                       of all possible query minimizers when i<1) for aligning the pattern with \n"
				         "                       the read [%g]\n", opt.max_seeds);
		fprintf(fp_help, "=======Compressed Seeding=======\n");
		fprintf(fp_help, "    -f FLOAT           filter out top FLOAT fraction of repetitive minimizers [%g]\n", 		
												 opt.mid_occ_frac);
		fprintf(fp_help, "=======Location Voting=======\n");
		fprintf(fp_help, "    --sort STR         sorting algorithm for sorting the output of index queyring. \n"
				         "                       Use heap, radix, or merge [none]\n");
//		fprintf(fp_help, "    -n FLOAT[,FLOAT]   retain all mapping locations that have at least FLOAT1 seed matches, \n"
//						 "                       if not found retain only a single mapping location tat has FLOAT2 seed \n"
//						 "                       matches. It can be as absolute number when n>=1 or fraction of all possible \n"
//				         "                       query minimizers when n<1.  [%g, %g]\n", opt.min_cnt, opt.rec_threshold_frac);
		//fprintf(fp_help, "    --AF_dis FLOAT    maximum length (AF_dis * read length) of a region span for Adjacency "
		//										"Filtering [%g]\n",
		//										opt.AF_dis);
		fprintf(fp_help, "    --vt_nb_loc UINT   retain at most UINT mapping locations for both rounds of location voting [%u]\n", opt.vt_nb_loc);
		fprintf(fp_help, "    --vt_df1 FLOAT     retain all mapping locations for 1st voting round, where each location \n"
				         "                       has at least (FLOAT*mapping distance) seed matches [%f]\n", opt.vt_df1);
		fprintf(fp_help, "    --vt_df2 FLOAT     retain all mapping locations for 2nd voting round, where each location \n"
				         "                       has at least (FLOAT*mapping distance) seed matches [%f]\n", opt.vt_df2);
		fprintf(fp_help, "    --vt_cov FLOAT     coverage threshold for 2nd voting round [%f]\n", opt.vt_cov);
		fprintf(fp_help, "    --max_min_gap UINT minimum gap size for performing splice alignment [%u]\n", opt.max_min_gap);
		fprintf(fp_help, "    --max_max_gap UINT maximum gap size for performing splice alignment [%u]\n", opt.max_max_gap);
		fprintf(fp_help, "    --vt_dis UINT      voting distance [%u]\n", opt.vt_dis);
		//fprintf(fp_help, "    --AF_max_loc INT   retain at most INT mapping locations for alignment [%d]\n", opt.AF_max_loc);
		fprintf(fp_help, "=======Sequence Alignment=======\n");
		fprintf(fp_help, "    -r NUM             alignment bandwidth / edit distance [%d]\n", opt.bw);
		...
		//fprintf(fp_help, "    -r FLOAT[,INT,INT] edit distance threshold as percentage of read length, minimum alignment bandwidth, and maximum alignment bandwidth [%f,%d,%d]\n", opt.bw_frac, opt.bw_min, opt.bw_max);
		fprintf(fp_help, "    -s INT             minimal peak DP alignment score [%d]\n", opt.min_dp_max);
		// fprintf(fp_help, "    -n INT            minimal number of minimizers on a chain [%d]\n", opt.min_cnt);
		//fprintf(fp_help,
		 //       "    -m INT       minimal chaining score (matching bases minus log gap penalty) [%d]\n",
		  //      opt.min_chain_score);
		...
		//fprintf(fp_help, "\nSee `man ./minimap2.1' for detailed description of these and other advanced "
	...
	if (opt.best_n == 0 && (opt.flag & MM_F_CIGAR) && mm_verbose >= 2)
		fprintf(stderr, "[WARNING]\033[1;31m `-N 0' reduces alignment accuracy. Please use --secondary=no to "
		                "suppress secondary alignments.\033[0m\n");
		...
		if ((opt.flag & MM_F_OUT_SAM) && idx_rdr->n_parts == 1) {
			if (mm_idx_reader_eof(idx_rdr)) {
				...
				if (opt.split_prefix == 0 && mm_verbose >= 2)
					fprintf(stderr, "[WARNING]\033[1;31m For a multi-part index, no @SQ lines will "
					                "be outputted. Please use --split-prefix.\033[0m\n");
			}
			if (ret != 0) {
				mm_idx_destroy(mi);
				mm_idx_reader_close(idx_rdr);
	...
	if (opt.split_prefix) mm_split_merge(argc - (o.ind + 1), (const char **)&argv[o.ind + 1], &opt, n_parts);
	...
	if (mm_verbose >= 3) {
		...
		fprintf(stderr, "\n[M::%s] Real time: %.3f sec; CPU: %.3f sec; Peak RSS: %.3f GB\n", __func__,
		        realtime() - mm_realtime0, cputime(), peakrss() / 1024.0 / 1024.0 / 1024.0);
	}
	print_profile();
	return 0;
}
