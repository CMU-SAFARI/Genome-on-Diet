...
#define MM_F_RADIX_SORT (0x800000000LL)
...
// flexible array element for Sketch2
typedef struct {
	uint32_t n; // number of elements in shift_seeds_number[]
	uint32_t *shift_seeds_number;
} mm_pattern_t;
...
typedef struct {
	...
	int32_t dp_score, dp_max,
	    dp_max2; // DP score; score of the max-scoring segment; score of the best alternate mappings
	uint32_t n_ambi : 30,
	    trans_strand : 2; // number of ambiguous bases; transcript strand: 0 for unknown, 1 for +, 2 for -
...
typedef struct {
	...
	uint32_t mapq : 8, split : 2, rev : 1, inv : 1, sam_pri : 1, proper_frag : 1, pe_thru : 1, seg_split : 1,
	    seg_id : 8, split_inv : 1, is_alt : 1, strand_retained : 1, dummy : 5;
	uint32_t hash;
	float div;
	mm_extra_t *p;
} mm_reg1_t;
...
typedef struct {
	...
	float min_cnt; // min number of minimizers on each chain
	float rec_threshold_frac;
	...
	unsigned vt_nb_loc;
	float vt_cov;
	unsigned max_max_gap;
	unsigned max_min_gap;
	float vt_f;
	float vt_df1;
	float vt_df2;
...
extern int mm_verbose,
    mm_dbg_flag; // verbose level: 0 for no info, 1 for error, 2 for warning, 3 for message (default); debugging
                 // flag
