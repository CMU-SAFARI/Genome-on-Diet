...
void mm_map_frag(const mm_idx_t *mi, int n_segs, const int *qlens, const char **seqs, int *n_regs, mm_reg1_t **regs,
	...
	for (unsigned i = 0; i < vt.nb_seqs; i++) {
		...
		if (vt.seqs[i].first_query_loc < qrstart) {
			qrstart = vt.seqs[i].first_query_loc;
		}
		if (vt.seqs[i].last_query_loc > qrend) {
			qrend = vt.seqs[i].last_query_loc;
		}
	}

	// Perform a second round of voting if we don't cover the entire read
	if (qrstart > coverage_threshold) {
		vt_t vt2 = {.score = 0};
		vote_2(a_for, n_a_for, 0, &vt2, opt->vt_dis, tmp_extracted_len, 0, qrstart);
		vote_2(a_rev, n_a_rev, 1, &vt2, opt->vt_dis, tmp_extracted_len, 0, qrstart);
		if (mm_dbg_flag & MM_DBG_PRINT_SEED) {
	...
	if (qlen_sum - qrend > coverage_threshold) {
		vt_t vt2 = {.score = 0};
		vote_2(a_for, n_a_for, 0, &vt2, opt->vt_dis, tmp_extracted_len, qrend, qlen_sum);
		vote_2(a_rev, n_a_rev, 1, &vt2, opt->vt_dis, tmp_extracted_len, qrend, qlen_sum);
		if (mm_dbg_flag & MM_DBG_PRINT_SEED) {
			const char *chrom_name = mi->seq[vt2.chrom_id].name;
			fprintf(stderr, "VT2: score: %u, chrom: %s, loc: %u, df2: %f\n", vt2.score, chrom_name,
			        vt2.first_target_loc, opt->vt_df2);
	...
	*/
	...
	for (unsigned i = 0; i < vt.nb_seqs; i++) {
		if (vt.seqs[i].valid) {
			//printf("%d\n",vt.seqs[i].r.p->dp_score);
			if (vt.seqs[i].r.p->dp_score < opt->min_dp_max)
				vt.seqs[i].valid=0;
			else
				nb_out++;
		}
