...
static inline void update_max_zdrop(int32_t score, int i, int j, int32_t *max, int *max_i, int *max_j, int e,
                                    int *max_zdrop, int pos[2][2]) {
	if (score < *max) {
		int li   = i - *max_i;
		int lj   = j - *max_j;
...
static int mm_test_zdrop(void *km, const mm_mapopt_t *opt, const uint8_t *qseq, const uint8_t *tseq, uint32_t n_cigar,
                         uint32_t *cigar, const int8_t *mat) {
	uint32_t k;
	...
	if (!(opt->flag & (MM_F_SPLICE | MM_F_SR | MM_F_FOR_ONLY | MM_F_REV_ONLY)) && max_zdrop > opt->zdrop_inv &&
	    q_len < opt->max_gap && t_len < opt->max_gap) {
		uint8_t *qseq2;
		void *qp;
		int q_off, t_off;
	...
	*qshift = *tshift = 0;
	...
	for (k = 0; k < p->n_cigar; ++k) { // indel left alignment
		...
		if (op == MM_CIGAR_MATCH) {
			...
			if (k > 0 && k < p->n_cigar - 1 && (p->cigar[k - 1] & 0xf) == 0 &&
			    (p->cigar[k + 1] & 0xf) == 0) {
				int l, prev_len = p->cigar[k - 1] >> 4;
				if (op == MM_CIGAR_INS) {
					for (l = 0; l < prev_len; ++l)
						if (qseq[qoff - 1 - l] != qseq[qoff + len - 1 - l]) break;
				} else {
					for (l = 0; l < prev_len; ++l)
						if (tseq[toff - 1 - l] != tseq[toff + len - 1 - l]) break;
				}
				if (l > 0) p->cigar[k - 1] -= l << 4, p->cigar[k + 1] += l << 4, qoff -= l, toff -= l;
	...
	for (k = 0; k < r->p->n_cigar; ++k) {
		...
		if (op == MM_CIGAR_MATCH) {
			while (len > 0) {
				...
				} // run of "="; TODO: N<=>N is converted to "="
				if (l > 0) {
					++n_EQX;
					len -= l;
					toff += l;
...
void mm_update_extra(mm_reg1_t *r, const uint8_t *qseq, const uint8_t *tseq, const int8_t *mat, int8_t q, int8_t e,
	...
	assert(qoff == r->qe - r->qs && toff == r->re - r->rs);
	if (is_eqx)
		mm_update_cigar_eqx(r, qseq,
		                    tseq); // NB: it has to be called here as changes to qseq and tseq are not returned
}
...
static void mm_align_pair(void *km, const mm_mapopt_t *opt, int qlen, const uint8_t *qseq, int tlen,
	...
	if (opt->max_sw_mat > 0 && (int64_t)tlen * qlen > opt->max_sw_mat) {
		...
		ksw_exts2_sse(km, qlen, qseq, tlen, tseq, 5, mat, opt->q, opt->e, opt->q2, opt->noncan, zdrop,
		              opt->junc_bonus, flag, junc, ez);
...
static int *collect_long_gaps(void *km, int as1, int cnt1, mm128_t *a, int min_gap, int *n_) {
	...
	*n_ = 0;
	...
	for (i = 1, n = 0; i < cnt1; ++i) { // store the positions of long gaps
		int gap = ((int32_t)a[as1 + i].y - a[as1 + i - 1].y) - ((int32_t)a[as1 + i].x - a[as1 + i - 1].x);
		if (gap < -min_gap || gap > min_gap) K[n++] = i;
	}
	*n_ = n;
	return K;
}

static void mm_filter_bad_seeds(void *km, int as1, int cnt1, mm128_t *a, int min_gap, int diff_thres, int max_ext_len,
                                int max_ext_cnt) {
	...
	for (k = 0;; ++k) { // traverse long gaps
		...
		for (l = k + 1; l < n && l <= k + max_ext_cnt; ++l) {
			...
			gap = ((int32_t)a[as1 + j].y - (int32_t)a[as1 + j - 1].y) -
			      (int32_t)(a[as1 + j].x - a[as1 + j - 1].x);
			if (gap > 0)
				n_ins += gap;
			else
				n_del += -gap;
			diff = n_ins + n_del - abs(n_ins - n_del);
			if (max_diff < diff) max_diff = diff, max_diff_l = l;
		}
		if (max_diff > diff_thres && max_diff > max) max = max_diff, max_st = k, max_en = max_diff_l;
	}
	kfree(km, K);
}
...
static void mm_filter_bad_seeds_alt(void *km, int as1, int cnt1, mm128_t *a, int min_gap, int max_ext) {
	...
	for (k = 0; k < n;) {
		int i    = K[k], l;
		int gap1 = ((int32_t)a[as1 + i].y - (int32_t)a[as1 + i - 1].y) -
		           ((int32_t)a[as1 + i].x - (int32_t)a[as1 + i - 1].x);
		...
		for (l = k + 1; l < n; ++l) {
			...
			gap2 = ((int32_t)a[as1 + j].y - (int32_t)a[as1 + j - 1].y) -
			       (int32_t)(a[as1 + j].x - a[as1 + j - 1].x);
	...
	*as = r->as, *cnt = r->cnt;
	...
	for (i = r->as + 1; i < r->as + r->cnt; ++i) {
		...
		if (lq == lr) {
			...
			if (score > max_score) max_score = score, max_len = len, max_i = i - len;
			score = q_span, len = 1;
		}
	}
	if (score > max_score) max_score = score, max_len = len, max_i = i - len;
...
static int mm_seed_ext_score(void *km, const mm_mapopt_t *opt, const mm_idx_t *mi, const int8_t mat[25], int qlen,
                             uint8_t *qseq0[2], const mm128_t *a) {
	uint8_t *qseq, *tseq;
...
static void mm_fix_bad_ends_splice(void *km, const mm_mapopt_t *opt, const mm_idx_t *mi, const mm_reg1_t *r,
                                   const int8_t mat[25], int qlen, uint8_t *qseq0[2], const mm128_t *a, int *as1,
                                   int *cnt1) { // this assumes a very crude k-mer based mode; it is not necessary to
	                                        // use a good model just for filtering bounary exons
	int score;
	double log_gap;
	*as1 = r->as, *cnt1 = r->cnt;
	if (r->cnt < 3) return;
	...
	if ((a[r->as].y >> 32 & 0xff) < log_gap + opt->anchor_ext_shift) {
		...
		if ((double)score / mat[0] <
		    log_gap + opt->anchor_ext_shift) // a more exact format is "score < log_4(gap) + shift"
			++(*as1), --(*cnt1);
	}
	...
	if ((a[r->as + r->cnt - 1].y >> 32 & 0xff) < log_gap + opt->anchor_ext_shift) {
		...
		if ((double)score / mat[0] < log_gap + opt->anchor_ext_shift) --(*cnt1);
	}
}

static void mm_align1(void *km, const mm_mapopt_t *opt, const mm_idx_t *mi, int qlen, uint8_t *qseq0[2], mm_reg1_t *r,
                      mm_reg1_t *r2, int n_a, mm128_t *a, ksw_extz_t *ez, int splice_flag) {
	...
	if (is_sr)
		assert(!(mi->flag &
		         MM_I_HPC)); // HPC won't work with SR because with HPC we can't easily tell if there is a gap

	r2->cnt = 0;
	if (r->cnt == 0) return;
	ksw_gen_simple_mat(5, mat, opt->a, opt->b, opt->sc_ambi);
	bw      = (int)(opt->bw * 1.5 + 1.);
	bw_long = (int)(opt->bw * 1.5 + 1.);
	...
	if (is_sr) {
		...
		for (i = r->as - 1, l = 0; i >= 0 && a[i].x >> 32 == a[r->as].x >> 32; --i) { // inspect nearby seeds
			...
			if (x < rs0 && y < qs0) {
				if (++l > opt->min_cnt) {
					...
					rs1 = rs0 - l, qs1 = qs0 - l;
					if (rs1 < 0)
						rs1 = 0; // not strictly necessary; better have this guard for explicit
					break;
				}
			}
		}
		...
		for (i = r->as + r->cnt, l = 0; i < n_a && a[i].x >> 32 == a[r->as].x >> 32;
		     ++i) { // inspect nearby seeds
	...
	if (qs > 0 && rs > 0) { // left extension; probably the condition can be changed to "qs > qs0 && rs > rs0"
		...
		mm_align_pair(km, opt, qs - qs0, qseq, rs - rs0, tseq, junc, mat, bw, opt->end_bonus,
		              r->split_inv ? opt->zdrop_inv : opt->zdrop,
		              extra_flag | KSW_EZ_EXTZ_ONLY | KSW_EZ_RIGHT | KSW_EZ_REV_CIGAR, ez);
		if (ez->n_cigar > 0) {
	...
	for (i = is_sr ? cnt1 - 1 : 1; i < cnt1; ++i) { // gap filling
		...
		if (i == cnt1 - 1 || (a[as1 + i].y & MM_SEED_LONG_JOIN) ||
			...
			if (a[as1 + i].y & MM_SEED_LONG_JOIN) bw1 = qe - qs > re - rs ? qe - qs : re - rs;
			// perform alignment
			if (opt->flag & MM_F_QSTRAND) {
				qseq = &qseq0[0][qs];
			...
			if (is_sr) { // perform ungapped alignment
				...
				ez->cigar =
				    ksw_push_cigar(km, &ez->n_cigar, &ez->m_cigar, ez->cigar, MM_CIGAR_MATCH, qe - qs);
			} else { // perform normal gapped alignment
				mm_align_pair(km, opt, qe - qs, qseq, re - rs, tseq, junc, mat, bw1, -1, opt->zdrop,
				              extra_flag | KSW_EZ_APPROX_MAX,
				              ez); // first pass: with approximate Z-drop
			}
			// test Z-drop and inversion Z-drop
			if ((zdrop_code = mm_test_zdrop(km, opt, qseq, tseq, ez->n_cigar, ez->cigar, mat)) != 0)
				mm_align_pair(km, opt, qe - qs, qseq, re - rs, tseq, junc, mat, bw1, -1,
				              zdrop_code == 2 ? opt->zdrop_inv : opt->zdrop, extra_flag,
				              ez); // second pass: lift approximate
			// update CIGAR
			if (ez->n_cigar > 0) mm_append_cigar(r, ez->n_cigar, ez->cigar);
			if (ez->zdropped) { // truncated by Z-drop; TODO: sometimes Z-drop kicks in because the next
				            // seed placement is wrong. This can be fixed in principle.
				if (!r->p) {
					assert(ez->n_cigar == 0);
	...
	if (!dropped && qe < qe0 && re < re0) { // right extension
		...
		mm_align_pair(km, opt, qe0 - qe, qseq, re0 - re, tseq, junc, mat, bw, opt->end_bonus, opt->zdrop,
		              extra_flag | KSW_EZ_EXTZ_ONLY, ez);
		if (ez->n_cigar > 0) {
			mm_append_cigar(r, ez->n_cigar, ez->cigar);
			r->p->dp_score += ez->max;
		}
	...
	if (r->p) {
		...
		if (rev && r->p->trans_strand) r->p->trans_strand ^= 3; // flip to the read strand
	}
...
static int mm_align1_inv(void *km, const mm_mapopt_t *opt, const mm_idx_t *mi, int qlen, uint8_t *qseq0[2],
                         const mm_reg1_t *r1, const mm_reg1_t *r2, mm_reg1_t *r_inv,
                         ksw_extz_t *ez) { // NB: this doesn't work with the qstrand mode
	...
	mm_align_pair(km, opt, ql - q_off, qseq + q_off, tl - t_off, tseq + t_off, 0, mat, (int)(opt->bw * 1.5), -1,
	              opt->zdrop, KSW_EZ_EXTZ_ONLY, ez);
...
static inline mm_reg1_t *mm_insert_reg(const mm_reg1_t *r, int i, int *n_regs, mm_reg1_t *regs) {
	...
	if (i + 1 != *n_regs) memmove(&regs[i + 2], &regs[i + 1], sizeof(mm_reg1_t) * (*n_regs - i - 1));
	regs[i + 1] = *r;
	++*n_regs;
	return regs;
}
...
static inline void mm_count_gaps(const mm_reg1_t *r, int32_t *n_gap_, int32_t *n_gapo_) {
	...
	*n_gap_ = *n_gapo_ = -1;
	...
	for (i = 0; i < r->p->n_cigar; ++i) {
		...
		if (op == MM_CIGAR_INS || op == MM_CIGAR_DEL) ++n_gapo, n_gap += len;
	}
...
void mm_update_dp_max(int qlen, int n_regs, mm_reg1_t *regs, float frac, int a, int b) {
	...
	for (i = 0; i < n_regs; ++i) {
		...
		if (r->p == 0) continue;
		if (r->p->dp_max > max)
			max2 = max, max = r->p->dp_max, max_i = i;
...
mm_reg1_t *mm_align_skeleton(void *km, const mm_mapopt_t *opt, const mm_idx_t *mi, int qlen, const char *qstr,
	...
	for (i = 0; i < n_regs; ++i) {
		mm_reg1_t r2;
		if ((opt->flag & MM_F_SPLICE) && (opt->flag & MM_F_SPLICE_FOR) &&
		    (opt->flag & MM_F_SPLICE_REV)) { // then do two rounds of alignments for both strands
			mm_reg1_t s[2], s2[2];
			...
			if (s[0].p->dp_score > s[1].p->dp_score)
				which = 0, trans_strand = 1;
			else if (s[0].p->dp_score < s[1].p->dp_score)
				which = 1, trans_strand = 2;
			else
				trans_strand = 3,
				which        = (qlen + s[0].p->dp_score) & 1; // randomly choose a strand, effectively
			if (which == 0) {
				regs[i] = s[0], r2 = s2[0];
				free(s[1].p);
			} else {
			...
			if (opt->flag & MM_F_SPLICE) regs[i].p->trans_strand = opt->flag & MM_F_SPLICE_FOR ? 1 : 2;
		}
