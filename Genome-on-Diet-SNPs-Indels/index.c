...
void mm_idx_stat(const mm_idx_t *mi) {
	...
	fprintf(stderr,
	        "[M::%s::%.3f*%.2f] distinct minimizers: %d (%.2f%% are singletons); average occurrences: %.3lf; "
	        "average spacing: %.3lf; total length: %ld\n",
	        __func__, realtime() - mm_realtime0, cputime() / (realtime() - mm_realtime0), n, 100.0 * n1 / n,
	        (double)sum / n, (double)len / sum, (long)len);
}
	...
	for (j = 1, n = 1, start_a = start_p = 0; j <= b->a.n; ++j) {
		if (j == b->a.n || b->a.a[j].x >> 8 != b->a.a[j - 1].x >> 8) {
			...
			if (n == 1) {
				...
				radix_sort_64(&b->p[start_p],
				              &b->p[start_p + n]); // sort by position; needed as in-place
				                                   // radix_sort_128x() is not stable
...
static void mm_idx_post(mm_idx_t *mi, int n_threads) { kt_for(n_threads, worker_post, mi, 1 << mi->b); }
	...
	if (step == 0) { // step 0: read sequences
		...
		if (s->seq) {
			...
			for (i = 0; i < s->n_seq; ++i) {
				...
				// copy the sequence
				if (!(p->mi->flag & MM_I_NO_SEQ)) {
					for (j = 0; j < seq->len; ++j) { // TODO: this is not the fastest way, but let's
						                         // first see if speed matters here
		...
		for (i = 0; i < s->n_seq; ++i) {
			...
			if (t->l_seq > 0) {
				mm_sketch(0, t->seq, t->l_seq, p->mi->w, p->mi->k, t->rid, p->mi->flag & MM_I_HPC,
				          &s->a, p->pattern, p->pattern_len);
...
mm_idx_t *mm_idx_gen(mm_bseq_file_t *fp, int w, int k, int b, int flag, int mini_batch_size, int n_threads,
                     uint64_t batch_size, char *pattern, int pattern_len) {
	pipeline_t pl;
...
mm_idx_t *mm_idx_str(int w, int k, int is_hpc, int bucket_bits, int n, const char **seq, const char **name) {
	...
	for (i = 0, sum_len = 0; i < n; ++i) {
		...
		if (p->len > 0) {
			...
			char *T = 0;
			int N   = 0;
			mm_sketch(0, s, p->len, w, k, i, is_hpc, &a, T, N);
...
void mm_idx_dump(FILE *fp, const mm_idx_t *mi) {
	...
	if (!(mi->flag & MM_I_NO_SEQ)) fwrite(mi->S, 4, (sum_len + 7) / 8, fp);
	fflush(fp);
}
...
#endif // WIN32
		ret = read(fd, magic, 4);
		if (ret == 4 && strncmp(magic, MM_IDX_MAGIC, 4) == 0) is_idx = 1;
	}
	close(fd);
...
mm_idx_t *mm_idx_reader_read(mm_idx_reader_t *r, int n_threads) {
	...
	if (r->is_idx) {
		...
		if (mi && mm_verbose >= 2 &&
			...
			fprintf(stderr, "[WARNING]\033[1;31m Indexing parameters (-k, -w or -H) overridden by "
			                "parameters used in the prebuilt index.\033[0m\n");
	} else
		mi = mm_idx_gen(r->fp.seq, r->opt.w, r->opt.k, r->opt.bucket_bits, r->opt.flag, r->opt.mini_batch_size,
		                n_threads, r->opt.batch_size, r->opt.pattern, r->opt.pattern_len);
	if (mi) {
...
int mm_idx_alt_read(mm_idx_t *mi, const char *fn) {
	...
	mi->n_alt = n_alt;
	if (mm_verbose >= 3) fprintf(stderr, "[M::%s] found %d ALT contigs\n", __func__, n_alt);
	return n_alt;
}
