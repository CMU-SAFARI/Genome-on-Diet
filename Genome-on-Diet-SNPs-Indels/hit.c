...
static inline void
mm_reg_set_coor(mm_reg1_t *r, int32_t qlen, const mm128_t *a,
                int is_qstrand) { // NB: r->as and r->cnt MUST BE set correctly for this function to work
	int32_t k = r->as, q_span = (int32_t)(a[k].y >> 32 & 0xff);
	r->rev = a[k].x >> 63;
	r->rid = a[k].x << 1 >> 33;
	r->rs  = (int32_t)a[k].x + 1 > q_span ? (int32_t)a[k].x + 1 - q_span
	                                      : 0; // NB: target span may be shorter, so this test is necessary
...
void mm_mark_alt(const mm_idx_t *mi, int n, mm_reg1_t *r) {
	...
	for (i = 0; i < n; ++i)
		if (mi->seq[r[i].rid].is_alt) r[i].is_alt = 1;
}
                   ...
                   float alt_diff_frac) // and compute mm_reg1_t::subsc
{
	...
	for (i = 1, k = 1; i < n; ++i) {
		...
		} else if (n_cov > 0) { // there are overlapping primary hits; find the length not covered by existing
			                // primary hits
			int j, x = si;
	...
	skip_uncov:
		for (j = 0; j < k; ++j) { // traverse existing primary hits again
			...
			ol  = si < sj ? (ei < sj   ? 0
			              ...
			              : (ej < si   ? 0
			                 : ej < ei ? ej - si
			                           : ei - si); // overlap length; TODO: this can be simplified
			if ((float)ol / min - (float)uncov_len / max > mask_level &&
			    uncov_len <= mask_len) { // then this is a secondary hit
				...
				if (rp->p && ri->p &&
				    (rp->rid != ri->rid || rp->rs != ri->rs || rp->re != ri->re ||
				     ol != min)) { // the last condition excludes identical hits after DP
					sci = ri->p->dp_max;
					if (!rp->is_alt && ri->is_alt) sci = mm_alt_score(sci, alt_diff_frac);
...
void mm_hit_sort(void *km, int *n_regs, mm_reg1_t *r, float alt_diff_frac) {
	...
	for (i = n_aux = 0; i < n; ++i) {
		if (r[i].inv || r[i].cnt > 0) { // squeeze out elements with cnt==0 (soft deleted)
			int score;
			if (r[i].p)
				score = r[i].p->dp_max, has_cigar = 1;
			else
				score = r[i].score, no_cigar = 1;
...
void mm_sync_regs(void *km, int n_regs, mm_reg1_t *regs) // keep mm_reg1_t::{id,parent} in sync; also reset id
{
	...
	for (i = 0; i < n_regs; ++i) {
		...
		if (r->parent == MM_PARENT_TMP_PRI)
			...
			r->parent = MM_PARENT_UNSET;
	}
	kfree(km, tmp);
	mm_set_sam_pri(n_regs, regs);
}

void mm_select_sub(void *km, float pri_ratio, int min_diff, int best_n, int check_strand, int min_strand_sc, int *n_,
                   mm_reg1_t *r) {
	if (pri_ratio > 0.0f && *n_ > 0) {
		...
		for (i = k = 0; i < n; ++i) {
			...
			if (p == i || r[i].inv) { // primary or inversion
				r[k++] = r[i];
			} else if ((r[i].score >= r[p].score * pri_ratio || r[i].score + min_diff >= r[p].score) &&
			           n_2nd < best_n) {
				if (!(r[i].qs == r[p].qs && r[i].qe == r[p].qe && r[i].rid == r[p].rid &&
				      r[i].rs == r[p].rs && r[i].re == r[p].re)) // not identical hits
					r[k++] = r[i], ++n_2nd;
				else if (r[i].p)
					free(r[i].p);
			} else if (check_strand && n_2nd < best_n && r[i].score > min_strand_sc &&
			           r[i].rev != r[p].rev) {
...
void mm_filter_regs(const mm_mapopt_t *opt, int qlen, int *n_regs,
                    mm_reg1_t *regs) { // NB: after this call, mm_reg1_t::parent can be -1 if its parent filtered out
	int i, k;
	for (i = k = 0; i < *n_regs; ++i) {
		mm_reg1_t *r = &regs[i];
		int flt      = 0;
...
int mm_squeeze_a(void *km, int n_regs, mm_reg1_t *regs,
                 mm128_t *a) { // squeeze out regions in a[] that are not referenced by regs[]
	int i, as = 0;
	uint64_t *aux;
...
mm_seg_t *mm_seg_gen(void *km, uint32_t hash, int n_segs, const int *qlens, int n_regs0, const mm_reg1_t *regs0,
	...
	for (s = 0; s < n_segs; ++s) {
		...
		for (i = 0, sr->n_u = 0; i < n_regs0; ++i) // squeeze out zero-length per-segment chains
			if ((int32_t)sr->u[i] != 0) sr->u[sr->n_u++] = sr->u[i];
	...
	for (i = 0; i < n_regs0; ++i) {
		...
		for (j = 0; j < r->cnt; ++j) {
			...
			//   (int32_t)new_a1.y = qlens[sid] - (x_for_cat - acc_qlen[sid] + 1 - q_span) - 1 =
			//   (int32_t)a1.y - (qlen_sum - (qlens[sid] + acc_qlen[sid]))
...
void mm_set_mapq(void *km, int n_regs, mm_reg1_t *regs, int min_chain_sc, int match_sc, int rep_len, int is_sr) {
	...
	if (n_regs == 0) return;
	for (i = 0; i < n_regs; ++i)
		if (regs[i].parent == regs[i].id) sum_sc += regs[i].score;
	...
	for (i = 0; i < n_regs; ++i) {
		...
		if (r->inv) {
			...
			if (r->p && r->p->dp_max2 > 0 && r->p->dp_max > 0) {
				...
				mapq           = (int)(identity * pen_cm * q_coef * (1.0f - x * x) *
                                             logf((float)r->p->dp_max / match_sc));
				if (!is_sr) {
					int mapq_alt = (int)(6.02f * identity * identity *
					                         (r->p->dp_max - r->p->dp_max2) / match_sc +
					                     .499f); // BWA-MEM like mapQ, mostly for short reads
					mapq =
					    mapq < mapq_alt ? mapq : mapq_alt; // in case the long-read heuristic fails
				}
			} else {
				float x = (float)subsc / r->score0;
				if (r->p) {
					float identity = (float)r->mlen / r->blen;
					mapq           = (int)(identity * pen_cm * q_coef * (1.0f - x) *
                                                     logf((float)r->p->dp_max / match_sc));
				} else {
...
void mm_set_sam_params(void *km, const int n_regs, mm_reg1_t *const regs, const unsigned qlen,
                       const unsigned match_score, const unsigned max_nb_sec) {
	int supp_threshold = 0.8 * (float)(regs[0].qe - regs[0].qs);
	unsigned nb_sec    = 0;
	int dp_max2        = 0;
	regs[0].sam_pri    = 1;
	regs[0].parent     = regs[0].id;
	for (unsigned i = 1; i < n_regs; i++) {
		regs[i].sam_pri = 0;
		if (regs[i].qe - regs[i].qs > supp_threshold) {
			nb_sec++;
			regs[i].mapq   = 0;
			regs[i].parent = regs[i].id + 1;
			dp_max2        = regs[i].score;
		} else {
			regs[i].mapq   = 60;
			regs[i].parent = regs[i].id;
		}
	}

	// Sort secondaries and supplementaries
	for (unsigned i = 1; i < n_regs - 1; i++) {
		if (regs[i].parent != regs[i].id) {
			for (unsigned j = i + 1; j < n_regs; j++) {
				if (regs[j].parent == regs[j].id) {
					const mm_reg1_t tmp = regs[j];
					regs[j]             = regs[i];
					regs[i]             = tmp;
					break;
				} else {
	...
	if (nb_sec > 9) {
		...
		int dp_max     = regs[0].score;
		float identity = (float)regs[0].mlen / regs[0].blen;
		regs[0].mapq   = 54 * identity * (dp_max - dp_max2) / (qlen * match_score - dp_max2) + 5;
	} else {
		regs[0].mapq = 60;
	}
}
