...
static void collect_minimizers(void *km, const mm_mapopt_t *opt, const mm_idx_t *mi, int n_segs, const int *qlens, const char **seqs, mm128_v *mv, const char *Z, int W, int shift)
{
	int i, n, sum = 0;
	mv->n = 0;
	for (i = n = 0; i < n_segs; ++i) {
		size_t j;
		mm_sketch3(km, seqs[i], qlens[i], mi->w, mi->k, i, mi->flag&MM_I_HPC, mv, Z, W, shift);
		for (j = n; j < mv->n; ++j)
			mv->a[j].y += sum << 1;
...
mm_pattern_versions *collect_minimizers2(void *km, const mm_mapopt_t *opt, const mm_idx_t *mi, int n_segs, const int *qlens, const char **seqs, mm128_v *mv, const char *Z, int W, int max_seeds)
{
	int i, n;
	mv->n = 0;

	mm_pattern_versions *mm_pattern_versions1=0; 
	int *shift_seeds_number=0;
	for (i = n = 0; i < n_segs; ++i) {
		size_t j;
		mm_pattern_versions1 = mm_sketch2(km, seqs[i], qlens[i], mi->w, mi->k, i, mi->flag&MM_I_HPC, mv, Z, W, max_seeds);
		//fprintf(stderr, "Collect_minimizers2:\n");
		//printf("shift_seeds_number: %d, size: %d\n",mm_pattern_versions1->shift_seeds_number[0], mm_pattern_versions1->n_shift_seeds_number);
	}
	...
	while (heap_size > 0) {
		...
		if (!skip_seed(opt->flag, r, q, qname, qlen, mi, &is_self)) {
			if ((r&1) == (q->q_pos&1)) { // forward strand
				...
				//printf("Location: %d\n",((uint64_t)q->q_span << 32 | q->q_pos >> 1));
				//p->y = (uint64_t)q->q_span << 32 | (qlen - 1 - (q->q_pos>>1) - ((2*q->q_span)-2)); // convert e.g., 81 to 0 and 0 to 81
			}
...
void mm_map_frag(const mm_idx_t *mi, int n_segs, const int *qlens, const char **seqs, int *n_regs, mm_reg1_t **regs, mm_tbuf_t *b, const mm_mapopt_t *opt, const char *qname)
{
	...
	int shift = 0, m, n_m;
	mm_pattern_versions *mm_pattern_versions1 =0;
	mm_pattern_versions1= collect_minimizers2(b->km, opt, mi, n_segs, qlens, seqs, &mv, opt->pattern, opt->pattern_len, opt->max_seeds);
	shift = mm_collect_matches2(b->km, &n_m, qlen_sum, opt->mid_occ, opt->max_max_occ, opt->occ_dist, mi, &mv, &n_a, &rep_len, &n_mini_pos, &mini_pos, mm_pattern_versions1); // & or * or nothing
	//printf("Shift: %d\n", shift);
	//printf("%d\n\n\n\n", sizeof(shift_seeds_number)/sizeof(shift_seeds_number[0]));
	free(mm_pattern_versions1);

	collect_minimizers(b->km, opt, mi, n_segs, qlens, seqs, &mv, opt->pattern,  opt->pattern_len, shift);
	...
	int MAX_NUM_MAPPING_LOCATION_PER_READ = 10000;
	...
	double SEED_NUM_PER_READ = (opt->min_cnt > 1)?opt->min_cnt:(double)(((qlens[0]/opt->pattern_len)*Pattern_Num_Ones)-mi->k+1)/mi->w;
	double ReadSpan= (double)(qlens[0] * (1.20));
	int Edits =opt->bw;
	int MIN_SCORE=opt->min_dp_max;
	
	int init_loc_j=0, offset=0;
	int tl = qlens[0];
	int ql = qlens[0];
	ksw_extz_t ez;
	int sc_mch=opt->a; 
	...
	uint8_t *ts, *qs;
	uint8_t F2R[5]={3,2,1,0,4};

	//char *RefSeq = (char *)malloc(sizeof(char) * qlens[0]); // allocate memory for the segment of reference gneome
	uint8_t	c[256];
	memset(c, 4, 256);
	c['A'] = c['a'] = 0; c['C'] = c['c'] = 1;
	...
	for (j = 0; j < ql; ++j) {
		qs[j] = c[(uint8_t)seqs[0][j]];
	}

	// populate r[]
	mm_reg1_t *r;
	r = (mm_reg1_t*)calloc(MAX_NUM_MAPPING_LOCATION_PER_READ, sizeof(mm_reg1_t));

	if ((n_segs >= 1)&&(n_a>SEED_NUM_PER_READ)) { // uni-segment
		// This is considering the distance that distinct adjacent seeds spans compared to a read region = 120%*Read Length
		for (init_loc_j = 0, offset=1, Seed_Num=1; init_loc_j+offset < n_a;){ 
			if (MappedReadNo<MAX_NUM_MAPPING_LOCATION_PER_READ-1){
				//init_loc_j is first seed in a region, offset is iterating over the seeds in that region
				if((int32_t)a[init_loc_j+offset].x- (int32_t)a[init_loc_j].x <= ReadSpan) {
					if (a[init_loc_j+offset].x-a[init_loc_j+offset-1].x>0)
						Seed_Num++;
				} else{

					if(Seed_Num>=SEED_NUM_PER_READ) {
											//printf("Selected Mapping location is: %d, index: %d, Seed_Num: %d out of %f, Offset: %d, Strand: %c\n", (int32_t)a[init_loc_j].x, init_loc_j, Seed_Num, SEED_NUM_PER_READ, offset, "+-"[a[i].x>>63]);

						mappingStartingPosition = ((int32_t)a[init_loc_j].x);


						mapStartPos = mi->seq[a[init_loc_j].x<<1>>33].offset + (uint64_t)mappingStartingPosition;
						mapEndPos = (mapStartPos + (uint64_t)qlens[0]);


						int ttttt=0;
						if ((a[init_loc_j].x>>63)==0) {
							for (uint64_t mapSeqI = mapStartPos; mapSeqI < mapEndPos ; ++mapSeqI) {
								//RefSeq[mapSeqI-(mapStartPos)]="ACGTN"[mm_seq4_get(mi->S, mapSeqI)];
								ts[ttttt]=mm_seq4_get(mi->S, mapSeqI);
								ttttt=ttttt+1;
							}
							...
							for (uint64_t mapSeqI = mapEndPos-1 ; mapSeqI > mapStartPos; mapSeqI=mapSeqI-1) {
								...
								ts[ttttt]=F2R[mm_seq4_get(mi->S, mapSeqI)];
								ttttt=ttttt+1;
							}
						}
						...
						ksw_extz2_sse(0, ql, qs, tl, ts, 5, mat, opt->q, opt->e, opt->bw, opt->zdrop, opt->end_bonus, opt->flag, &ez);
						
						//if (ez.score >= MIN_SCORE) {
						//if (mm_dbg_flag & MM_DBG_PRINT_ALN_SEQ) {
						if (mm_dbg_flag & MM_DBG_PRINT_SEED) {
							printf("%d\t%d\t%d\n", mi->seq[a[init_loc_j].x<<1>>33].offset , mappingStartingPosition, mapStartPos);
							printf("%s\t%c\t%s\t%d\t%d\t", qname, '0', mi->seq[a[init_loc_j].x<<1>>33].name, mappingStartingPosition, ez.score);
							for (j = 0; j < ez.n_cigar; ++j){
								printf("%d%c", ez.cigar[j]>>4, "MIDNSH"[ez.cigar[j]&0xf]);
								//printf("%dM\t", r->qe - r->qs);
							}
							printf("\t%c\t%c\t%c\t%s\t%d\t%c\n", '*', '0', '0', seqs[0], Seed_Num, "+-"[(a[init_loc_j].x>>63)]);
						}

						mm_reg1_t *ri = &r[MappedReadNo];
						...
						if (ez.n_cigar >= 0) {
							if (!ri->p) {
								...
								ri->p->n_cigar = ez.n_cigar;
								ri->p->dp_max2 = ez.score;
							...
							p = ri->p;
							memcpy(p->cigar, ez.cigar, (ri->p->n_cigar )* 4);
							uint32_t k, l;
							...
							for (l = 0; l < qlens[0]; ++l) {
								if (ts[l] ==4 || qs[l] ==4) ++n_ambi;
								else if (ts[l] != qs[l]) ++n_diff;
							}
							...
							//p->n_cigar += ri->p->n_cigar;
						}

						ri->id = MappedReadNo;
						ri->parent = MappedReadNo;
						ri->score = ri->score0 = ql; //TODO
						ri->hash = (uint32_t)a[init_loc_j].x;
						ri->cnt = (int32_t)Seed_Num;
						ri->as = a[init_loc_j].y >> 32;
						ri->div = -1.0f;
						ri->qs = 0;
						ri->qe = ql;
						ri->rs = mappingStartingPosition;
						//ri->re = mapEndPos;
						ri->rev = a[init_loc_j].x>>63;
						ri->rid = a[init_loc_j].x<<1>>33;

						MappedReadNo=MappedReadNo+1;

						free(ez.cigar); 

						if (init_loc_j+offset < n_a){
							init_loc_j=init_loc_j+offset;
						}

					}else {
						if (init_loc_j+1 < n_a){
							init_loc_j=init_loc_j+1;
						}
					}
					offset=0;
					Seed_Num=1;
		...
		if(a[init_loc_j].x + qlens[0] <=mi->seq[a[init_loc_j].x<<1>>33].len){
			//printf("Selected Mapping location is: %d, index: %d, Seed_Num: %d out of %f, Offset: %d\n", (int32_t)a[init_loc_j].x, init_loc_j, Seed_Num, SEED_NUM_PER_READ, offset);
	...
	if (MappedReadNo<MAX_NUM_MAPPING_LOCATION_PER_READ)
		r = (mm_reg1_t*)realloc(r, (MappedReadNo) * sizeof(mm_reg1_t));

	n_regs[0] = MappedReadNo, regs[0] = r;
