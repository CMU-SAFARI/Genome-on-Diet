...
void mm_mapopt_init(mm_mapopt_t *opt) {
	...
	opt->bw = 0, opt->bw_min = 500, opt->bw_max = 1500, opt->bw_frac = 0.05;
	opt->max_gap          = 5000;
	opt->max_gap_ref      = -1;
...
void mm_mapopt_update(mm_mapopt_t *opt, const mm_idx_t *mi) {
	if ((opt->flag & MM_F_SPLICE_FOR) || (opt->flag & MM_F_SPLICE_REV)) opt->flag |= MM_F_SPLICE;
	if (opt->mid_occ <= 0) {
...
void mm_mapopt_max_intron_len(mm_mapopt_t *opt, int max_intron_len) {
	/*
	if ((opt->flag & MM_F_SPLICE) && max_intron_len > 0) opt->max_gap_ref = opt->bw = opt->bw_long = max_intron_len;
	*/
}
...
int mm_set_opt(const char *preset, mm_idxopt_t *io, mm_mapopt_t *mo) {
	if (preset == 0) {
		...
		if (strcmp(preset, "splice:hq") == 0) mo->junc_bonus = 5, mo->b = 4, mo->q = 6, mo->q2 = 24;
	} else
		return -1;
	return 0;
}
...
int mm_check_opt(const mm_idxopt_t *io, const mm_mapopt_t *mo) {
	...
	if (mo->bw > mo->bw_long) {
	        if (mm_verbose >= 1)
	                fprintf(
	                    stderr,
	                    "[ERROR]\033[1;31m with '-rNUM1,NUM2', NUM1 (%d) can't be larger than NUM2 (%d)\033[0m\n",
	                    mo->bw, mo->bw_long);
	        return -8;
	}
	*/
	...
	if (io->k <= 0 || io->w <= 0) {
		if (mm_verbose >= 1) fprintf(stderr, "[ERROR]\033[1;31m -k and -w must be positive\033[0m\n");
		return -5;
	}
	if (mo->best_n < 0) {
		if (mm_verbose >= 1) fprintf(stderr, "[ERROR]\033[1;31m -N must be no less than 0\033[0m\n");
		return -4;
	}
	if (mo->best_n == 0 && mm_verbose >= 2)
		fprintf(stderr, "[WARNING]\033[1;31m '-N 0' reduces mapping accuracy. Please use '--secondary=no' "
		                "instead.\033[0m\n");
	...
	if ((mo->flag & MM_F_FOR_ONLY) && (mo->flag & MM_F_REV_ONLY)) {
		if (mm_verbose >= 1)
			fprintf(
			    stderr,
			    "[ERROR]\033[1;31m --for-only and --rev-only can't be applied at the same time\033[0m\n");
		return -3;
	}
	if (mo->e <= 0 || mo->q <= 0) {
		if (mm_verbose >= 1) fprintf(stderr, "[ERROR]\033[1;31m -O and -E must be positive\033[0m\n");
		return -1;
	}
	if ((mo->q != mo->q2 || mo->e != mo->e2) && !(mo->e > mo->e2 && mo->q + mo->e < mo->q2 + mo->e2)) {
		if (mm_verbose >= 1)
			fprintf(stderr,
			        "[ERROR]\033[1;31m dual gap penalties violating E1>E2 and O1+E1<O2+E2\033[0m\n");
		return -2;
	}
	if ((mo->q + mo->e) + (mo->q2 + mo->e2) > 127) {
		if (mm_verbose >= 1)
			fprintf(stderr,
			        "[ERROR]\033[1;31m scoring system violating ({-O}+{-E})+({-O2}+{-E2}) <= 127\033[0m\n");
		return -1;
	}
	...
	if ((mo->flag & MM_F_NO_PRINT_2ND) && (mo->flag & MM_F_ALL_CHAINS)) {
		if (mm_verbose >= 1)
			fprintf(
			    stderr,
			    "[ERROR]\033[1;31m -X/-P and --secondary=no can't be applied at the same time\033[0m\n");
		return -5;
	}
	if ((mo->flag & MM_F_QSTRAND) &&
	    ((mo->flag & (MM_F_OUT_SAM | MM_F_SPLICE | MM_F_FRAG_MODE)) || (io->flag & MM_I_HPC))) {
		if (mm_verbose >= 1)
			fprintf(stderr,
			        "[ERROR]\033[1;31m --qstrand doesn't work with -a, -H, --frag or --splice\033[0m\n");
		return -5;
	}
	return 0;
}
