...
int main(int argc, char *argv[])
{
	...
	while ((c = ketopt(&o, argc, argv, 1, opt_str, long_options)) >= 0) {
		if (c == 'w') ipt.w = atoi(o.arg);
		...
		else if (c == 'Z') {opt.pattern = o.arg; ipt.pattern = o.arg;} // initialize the pattern
		else if (c == 'W') {opt.pattern_len = atoi(o.arg); ipt.pattern_len = atoi(o.arg);} // give pattern length
		else if (c == 'i') opt.max_seeds = atoi(o.arg); // max number of seeds for aligning the pattern with the read
			...
			opt.pattern_len = 2; ipt.pattern_len = 2; // disable the pattern
			opt.pattern="11"; ipt.pattern="11";
		} else if (opt.max_seeds<2) {
			opt.max_seeds =2;
	...
	if (argc == o.ind || fp_help == stdout) {
		...
		fprintf(fp_help, "    -i INT       maximum number of minimizers for aligning the pattern with the read [%d]\n", opt.max_seeds);
		fprintf(fp_help, "    -r NUM       alignment bandwidth / edit distance threshold [%d]\n", opt.bw);
		fprintf(fp_help, "    -n INT       minimal number of seed matches required for performing alignment [%d]\n", opt.min_cnt);
