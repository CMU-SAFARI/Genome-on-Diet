...
static int sam_write_rg_line(kstring_t *str, const char *s) {
	...
	if (strstr(s, "\t") != NULL) {
		if (mm_verbose >= 1)
			fprintf(stderr, "[ERROR] the read group line contained literal <tab> characters -- replace "
			                "with escaped tabs: \\t\n");
		goto err_set_rg;
	}
...
static void write_cs_core(kstring_t *s, const uint8_t *tseq, const uint8_t *qseq, const mm_reg1_t *r, char *tmp,
                          int no_iden, int write_tag) {
	int i, q_off, t_off;
	if (write_tag) mm_sprintf_lite(s, "\tcs:Z:");
	for (i = q_off = t_off = 0; i < (int)r->p->n_cigar; ++i) {
		int j, op = r->p->cigar[i] & 0xf, len = r->p->cigar[i] >> 4;
		assert((op >= MM_CIGAR_MATCH && op <= MM_CIGAR_N_SKIP) || op == MM_CIGAR_EQ_MATCH ||
		       op == MM_CIGAR_X_MISMATCH);
		if (op == MM_CIGAR_MATCH || op == MM_CIGAR_EQ_MATCH || op == MM_CIGAR_X_MISMATCH) {
			...
			assert(len >= 2);
			mm_sprintf_lite(s, "~%c%c%d%c%c", "acgtn"[tseq[t_off]], "acgtn"[tseq[t_off + 1]], len,
			                "acgtn"[tseq[t_off + len - 2]], "acgtn"[tseq[t_off + len - 1]]);
			t_off += len;
		}
	}
...
static void write_cs_or_MD(void *km, kstring_t *s, const mm_idx_t *mi, const mm_bseq1_t *t, const mm_reg1_t *r,
                           int no_iden, int is_MD, int write_tag, int is_qstrand) {
...
int mm_gen_cs_or_MD(void *km, char **buf, int *max_len, const mm_idx_t *mi, const mm_reg1_t *r, const char *seq,
                    int is_MD, int no_iden, int is_qstrand) {
	mm_bseq1_t t;
	kstring_t str;
...
static inline void write_tags(kstring_t *s, const mm_reg1_t *r) {
	...
	if (r->id == r->parent)
		...
		type = r->inv ? 'i' : 'S';
	if (r->p) {
		mm_sprintf_lite(s, "\tNM:i:%d\tms:i:%d\tAS:i:%d\tnn:i:%d", r->blen - r->mlen + r->p->n_ambi,
...
void mm_write_paf3(kstring_t *s, const mm_idx_t *mi, const mm_bseq1_t *t, const mm_reg1_t *r, void *km,
	...
	if (r->p && (opt_flag & (MM_F_OUT_CS | MM_F_OUT_MD)))
		write_cs_or_MD(km, s, mi, t, r, !(opt_flag & MM_F_OUT_CS_LONG), opt_flag & MM_F_OUT_MD, 1,
		               !!(opt_flag & MM_F_QSTRAND));
	if ((opt_flag & MM_F_COPY_COMMENT) && t->comment) mm_sprintf_lite(s, "\t%s", t->comment);
}
...
void mm_write_sam3(kstring_t *s, const mm_idx_t *mi, const mm_bseq1_t *t, int seg_idx, int reg_idx, int n_seg,
	...
	if (r == 0) {
		...
		if ((opt_flag & MM_F_LONG_CIGAR) && r->p && r->p->n_cigar > max_bam_cigar_op - 2) {
			...
			if (n_cigar > max_bam_cigar_op) cigar_in_tag = 1;
		}
		if (cigar_in_tag) {
			int slen;
			...
			clip_len[0] = r->rev ? qlen - r->qe : r->qs;
			clip_len[1] = r->rev ? r->qs : qlen - r->qe;
			fprintf(stderr, "clip_len[0]: %u, clip_len[1]: %u, qlen:%u\n", clip_len[0], clip_len[1], qlen);
			if (!(clip_len[0] < qlen && clip_len[1] < qlen)) {
			}
			*/
	...
	if (n_seg > 1) {
		...
		if (this_rid >= 0 && r_next) {
			...
			mm_sprintf_lite(s, "\t*\t0\t"); // neither has coordinates
		if (tlen > 0)
			++tlen;
		else if (tlen < 0)
			--tlen;
	...
	if (r) {
		write_tags(s, r);
		if (r->parent == r->id && r->p && n_regs > 1 && regs && r >= regs &&
		    r - regs < n_regs) { // supplementary aln may exist
			int i, n_sa = 0; // n_sa: number of SA fields
			for (i = 0; i < n_regs; ++i)
				if (i != r - regs && regs[i].parent == regs[i].id && regs[i].p) ++n_sa;
			if (n_sa > 0) {
		...
		if (cigar_in_tag) write_sam_cigar(s, flag, 1, t->l_seq, r, opt_flag);
	}
	if (rep_len >= 0) mm_sprintf_lite(s, "\trl:i:%d", rep_len);
...
void mm_write_sam2(kstring_t *s, const mm_idx_t *mi, const mm_bseq1_t *t, int seg_idx, int reg_idx, int n_seg,
                   const int *n_regss, const mm_reg1_t *const *regss, void *km, int64_t opt_flag) {
