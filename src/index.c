    ...
    if (step == 0) { // step 0: read sequences
		...
		for (i = 0; i < s->n_seq; ++i) {
			...
			if (t->l_seq > 0){
				char *T = 0;
				int N = 0;
				mm_sketch(0, t->seq, t->l_seq, p->mi->w, p->mi->k, t->rid, p->mi->flag&MM_I_HPC, &s->a, p->pattern, p->pattern_len);
			}
...
mm_idx_t *mm_idx_gen(mm_bseq_file_t *fp, int w, int k, int b, int flag, int mini_batch_size, int n_threads, uint64_t batch_size, char *pattern, int pattern_len)
{
	pipeline_t pl;
...
mm_idx_t *mm_idx_reader_read(mm_idx_reader_t *r, int n_threads)
{
	...
	if (r->is_idx) {
		...
		mi = mm_idx_gen(r->fp.seq, r->opt.w, r->opt.k, r->opt.bucket_bits, r->opt.flag, r->opt.mini_batch_size, n_threads, r->opt.batch_size, r->opt.pattern, r->opt.pattern_len);
	if (mi) {
