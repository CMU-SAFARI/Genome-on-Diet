...
static ko_longopt_t long_options[] = {{"bucket-bits", ko_required_argument, 300},
                                      ...
                                      {"AF_dis", ko_required_argument, 352},
                                      {"no-hash-name", ko_no_argument, 353},
                                      {"AF_max_loc", ko_required_argument, 354},
...
int main(int argc, char *argv[]) {
	...
	while ((c = ketopt(&o, argc, argv, 1, opt_str, long_options)) >= 0) {
		if (c == 'w')
			...
			opt.AF_dis = strtod(o.arg, NULL);
		} else if (c == 353)
			opt.flag |= MM_F_NO_HASH_NAME; // --no-hash-name
		else if (c == 354)
			opt.AF_max_loc = atof(o.arg); // --AF_max_loc
		else if (c == 330) {
			...
			if (x < 1.0) {
				opt.bw_frac = x;
				if (*p == ',') {
					opt.bw_min = (int)strtol(p + 1, &p, 10);
					if (*p == ',') {
						opt.bw_max = (int)strtol(p + 1, &p, 10);
					}
				}
			} else {
				opt.bw = (int)(x + .499);
	...
	if (argc == o.ind || fp_help == stdout) {
		...
		fprintf(fp_help, "    -n FLOAT[,FLOAT]   retain all mapping locations that have at least FLOAT1 seed matches, \n"
						 "                       if not found retain only a single mapping location that has FLOAT2 seed \n"
		...
		//fprintf(fp_help, "    -r NUM             alignment bandwidth / edit distance threshold [%d]\n", opt.bw);
		...
		fprintf(fp_help, "    -r FLOAT[,INT,INT] edit distance threshold as percentage of read length, minimum alignment bandwidth, \n"
				         "                       and maximum alignment bandwidth [%f,%d,%d]\n", opt.bw_frac, opt.bw_min, opt.bw_max);
