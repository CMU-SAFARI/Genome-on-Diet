...
int main(int argc, char *argv[]) {
	...
	if (argc == o.ind || fp_help == stdout) {
		fprintf(fp_help, "Usage: GDiet [options] <target.fa>|<target.idx> [query.fa] [...]\n");
		...
		fprintf(fp_help, "    -Z STR            pattern [%s]\n", ipt.pattern);
		fprintf(fp_help, "    -W INT            pattern length [%d]\n", ipt.pattern_len);
		fprintf(fp_help,
		        "    -i FLOAT          maximum number of minimizers (as absolute number when i>=1 or fraction "
		        "of all possible query minimizers when i<1) for aligning the pattern with the read [%g]\n",
		        opt.max_seeds);
		fprintf(fp_help,
		        "    -n FLOAT          minimal number of seed matches (as absolute number when n>=1 or "
		        "fraction of all possible query minimizers when n<1) required for performing alignment [%g]\n",
		        opt.min_cnt);
		fprintf(fp_help, "    --vt_dis UINT  voting distance [%u]\n", opt.vt_dis);
		fprintf(fp_help,
		        "    --vt_nb_loc UINT  maximum number of locations after the first round of voting [%d]\n",
		        opt.vt_dis);
		...
		fprintf(fp_help,
		        "    -E INT[,INT] gap extension penalty; a k-long gap costs min{O1+k*E1,O2+k*E2} [%d,%d]\n",
		        opt.e, opt.e2);
		fprintf(fp_help, "    -z INT[,INT] Z-drop score and inversion Z-drop score [%d,%d]\n", opt.zdrop,
		        opt.zdrop_inv);
		fprintf(fp_help, "    -u CHAR      how to find GT-AG. f:transcript strand, b:both strands, n:don't "
		                 "match GT-AG [n]\n");
